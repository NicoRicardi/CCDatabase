...
class DataCollector():
    ...
    def __init__(self,joblist=None, levelnames=["A","B","basis","calc"], 
        ...
        if parsername == None:  # process then use other args
            ...
            self.all_raws = [r[1] if type(r) in [tuple,list] else r for r in set(ittl.chain.from_iterable(reqsdict.values()))]
            self.parserdict = {rq : megadict[parsername]["parser"] for rq in self.all_raws}
...
#            self.parserargsdict = {rq: megadict[parsername]["parserargs"][rq] for rq in self.all_raws}
            ...
            self.parserargsdict = {rq:[{rq: megadict[parsername]["parser_args"][rq] for rq in self.all_raws}] for rq in self.all_raws}
            self.parserkwargsdict = {rq: megadict[parsername]["parser_kwargs"] for rq in self.all_raws}
        self.cqlist = cqlist 
...
ccpdict = {"reqs": {"ex_en": "exc_energy_rel"}, 
           ...
           "parser": None,
           "parser_args": {"exc_energy_rel":None},
